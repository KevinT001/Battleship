...
require './lib/board'
 ...
 RSpec.describe Board do 
    ...
    xit 'can check that coordinates are not diagonal' do 
        expect(board.valid_placement?(cruiser, ["A1", "B2", "C3"])).to eq false 
        expect(board.valid_placement?(submarine,["C2", "D3"])).to eq false
    end 

    xit 'can do a final check of valid coordinates for placement' do 
        expect(board.valid_placement?(submarine, ["A1", "A2"])).to eq true 
        expect(board.valid_placement?(cruiser, ["B1", "C2","D1"])).to eq true 
    end 
end
